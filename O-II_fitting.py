#This program fit all the O-III line with wavelength 5006.84 line using 10spaxel file. 
import numpy as np
...
x=np.linspace(4900,5200,240) #range of the x-axis
...
pdf = PdfPages("O-II_stdfitting.pdf")
...
for i in range (3364):
	...
	r=np.mean(dat)
	sl=dat[174]
	#sl=(dat[173]+2*dat[173]+3*dat[174])+2*dat[175]+dat[176] #sl denotes the slice number, and we used here "weighted average" concept
	guess=[abs(sl-r),5026.,2.,r] #guess the value for peak of curve which is intensity,lamda,width of lamda,background
	...
	plt.close()
pdf.close()
np.savetxt('O-II_outstd.txt', np.column_stack([p]), fmt=b'%10.6f')

...
x=np.linspace(4810,4950,114) #range of the x-axis
...
pdf = PdfPages("H_beta_newfit.pdf")
...
for i in range (3364):
	...
	dat=dat[xpos1:xpos2]
	r=np.mean(dat)
	sl=(dat[56]+3*dat[57]+dat[58])/5 #sl denotes the slice number, and we used here "weighted average" concept
	guess=[sl-r,4880,2,r] #guess the value for peak of curve which is intensity,lamda,width of lamda,background
	...
	plt.close()
pdf.close()
np.savetxt('H_beta_outnew.txt', np.column_stack([p]), fmt=b'%10.6f')
